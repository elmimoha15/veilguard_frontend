...
/**
 * Scanner (React Bits) — an animated WebGL scan-band field, used as the hero
 * background in Veilguard yellow on white. Adapted to TypeScript + a `'use client'`
 * boundary with an inline container style (no external CSS import). The canvas is
 * transparent (alpha), so it composites its yellow signal over the white page.
 */
...
export default function Scanner({
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    const u = ctx.program.uniforms as Uniforms;
  ...
  }, [speed, sweepSpeed, sweepWidth, sweepFalloff, scale, frequency, ripple, bandDensity, lineSharpness, glow, colorSpread, brightness, contrast, softness, vignette, opacity, scanline, grain, grainIntensity, scanDirection, mouseInteraction, mouseRadius, mouseStrength, color1, color2, color3]);
