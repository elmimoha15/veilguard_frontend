...
/**
 * GhostFibers (React Bits) — an animated WebGL nebula of glowing fibers, used as
 * the hero background in Veilguard yellow. Adapted to TypeScript, a `'use client'`
 * boundary, and inline container styles (no external CSS import).
 */
...
const setColor = (uniform: { value: unknown }, hex: string) => {
  const color = hexToRgb(hex);
  const arr = uniform.value as Float32Array;
  arr[0] = color[0];
  arr[1] = color[1];
  arr[2] = color[2];
};
...
const contexts = new WeakMap<HTMLElement, { program: Program; render: () => void; setPaused: (v: boolean) => void; setFps: (v: number) => void }>();
...
export default function GhostFibers({
  ...
  useEffect(() => {
    ...
    const setSize = () => {
      ...
      const res = uniforms.uResolution.value as Float32Array;
