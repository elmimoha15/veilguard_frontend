...
/**
 * Scanner — an animated WebGL "scan-line" field (React Bits, ported to TSX).
 * Renders a full-bleed <canvas> that draws interference bands sweeping across the
 * container. Meant as a decorative background layer (wrap in an aria-hidden,
 * absolutely-positioned parent). Pauses when offscreen or the tab is hidden, and
...
void main() {
  ...
  // Bright floor: the sweep gently brightens/dims but NEVER falls to black, so
  // the field reads as a constant moving background instead of a pulse.
  float sweep = mix(0.6, 1.0, pow(0.5 + 0.5 * cos(phase * TAU), max(uSweepFalloff, 0.1)));

  // Continuously scroll the scan lines with time so the background always drifts.
  float lc = coord * uBandDensity - t * 0.6;
...
export default function Scanner({
  ...
  useEffect(() => {
    ...
    // A missing/failed WebGL2 context must never break the page — the dark base
    // behind this layer simply shows through.
  ...
  useEffect(() => {
    ...
    // Reflect the latest uniforms immediately — matters when reduced-motion has
    // stopped the RAF loop (otherwise the single static frame keeps old colors).
