...
/**
 * MagicBento (React Bits, ported to TSX) — an interactive dark bento grid with
 * cursor spotlight, border glow, particle stars, tilt, magnetism and a click
 * ripple. Recolored to Veilguard yellow and filled with our scan capabilities.
 * Rendered as a dark accent band on the light landing (matches the reference).
 * Styles live in globals.css (.magic-bento-card / .card-grid / .particle etc.).
 */

const DEFAULT_PARTICLE_COUNT = 10;
const DEFAULT_SPOTLIGHT_RADIUS = 320;
const DEFAULT_GLOW_COLOR = '243, 197, 0'; // Veilguard yellow
...
const VEILGUARD_CARDS: BentoCard[] = [
  { label: 'Black-box', title: 'URL scan', description: 'Paste your live link. No install, no access, exactly what an attacker sees.' },
  { label: 'Connected', title: 'GitHub repo scan', description: 'Read-only deep scan of your source for keys, rules and access gaps.' },
  { label: 'Deep audit', title: 'Supabase & Firebase', description: 'Row-level security and database rules reviewed line by line, with the exact policy to paste.' },
  { label: 'Fix', title: 'The exact fix, not just the finding', description: 'Every issue ships with copy-paste code and a ready-made prompt for your AI tool.' },
  { label: 'Upload', title: 'File upload scan', description: 'Drag a folder or zip for a white-box scan.' },
  { label: 'Always on', title: 'Continuous monitoring', description: 'Re-scan on every deploy, instant email alerts.' },
];
    ...
    <>
      ...
      <div className="card-grid bento-section" ref={gridRef}>
        {cards.map((card, index) => {
          ...
          if (enableStars) {
            return (
              <ParticleCard key={index} className={baseClassName} style={style} disableAnimations={shouldDisableAnimations} particleCount={particleCount} glowColor={glowColor} enableTilt={enableTilt} clickEffect={clickEffect} enableMagnetism={enableMagnetism}>
                {inner}
              </ParticleCard>
            );
          }
