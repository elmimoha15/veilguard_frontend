...
import { SECURITY_ARTICLES, DRAFT_SLUGS, getArticle, articleHub } from '@/content/learn';

// Static export: pre-render exactly the security-library slugs, nothing else.
export const dynamicParams = false;

export function generateStaticParams() {
  return SECURITY_ARTICLES.map((a) => ({ slug: a.slug }));
}
...
export default async function SecurityArticle({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const a = getArticle(slug);
  if (!a || articleHub(slug) !== 'security') notFound();
